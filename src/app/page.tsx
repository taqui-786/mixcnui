  ...
  return (
   <div className="h-full w-full grid place-content-center">
    ...
    <motion.div 
      initial="initial"
      animate="animate"
      className="min-h-[80vh] flex items-center justify-center px-4 bg-white"
    >
      <motion.div variants={staggerChildren} className="max-w-4xl mx-auto text-center space-y-8">
        <motion.div variants={popIn} className="inline-flex items-center bg-gray-100 rounded-full px-4 py-1.5">
          <Terminal className="w-4 h-4 text-black mr-2" />
          <span className="text-sm font-medium text-gray-600">Ready to use components</span>
          <ChevronRight className="w-4 h-4 text-gray-400 ml-2" />
        </motion.div>

        <motion.h1 variants={popIn} className="text-5xl md:text-7xl font-bold text-black">
          shadcn/ui ∘ mixcn-ui
        </motion.h1>

        <motion.p variants={popIn} className="text-gray-600 text-lg max-w-2xl mx-auto">
          A perfect composition of beautiful components and powerful animations
        </motion.p>

        <motion.div variants={popIn} className="flex justify-center gap-4">
          <Link 
            href="/components" 
            className="group bg-black px-6 py-2.5 rounded-lg text-white hover:bg-black/90 transition-colors"
          >
          ...
          <Link 
            href="/docs" 
            className="px-6 py-2.5 rounded-lg border border-gray-200 text-black hover:bg-gray-50 transition-colors"
          >
            Documentation
          </Link>
        </motion.div>
      ...
      <motion.div variants={popIn}  className=" flex-row gap-2 mx-auto flex w-full max-w-[16rem] !mt-16  items-center justify-between">
